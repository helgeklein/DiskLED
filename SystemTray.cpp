                         ...
                         UINT uBalloonTimeout /*=10*/)
{
    ...
    _tcsncpy_s(m_tnd.szTip, 64, szToolTip, m_nMaxTooltipLength-1);
...
BOOL CSystemTray::SetTooltipText(CString pszTip)
{
	// Make sure the string is not longer than allowed by the shell API
	if (pszTip.GetLength () > (int) m_nMaxTooltipLength - 1)
	{
		pszTip.Truncate (m_nMaxTooltipLength - 1);
	}
    ...
    _tcsncpy_s (m_tnd.szTip, m_nMaxTooltipLength, pszTip, m_nMaxTooltipLength - 1);
